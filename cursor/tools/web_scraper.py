...
def get_atlassian_api_key_from_env() -> Optional[str]:
    """Retrieve the ATLASSIAN_API_KEY from environment variables."""
    return os.getenv('ATLASSIAN_API_KEY')


async def fetch_page(url: str, context, api_key: Optional[str]) -> Optional[str]:
    """Asynchronously fetch a webpage's content."""
    page = await context.new_page()
    try:
        logger.info(f"Fetching {url}")
        # Check if the URL is an Atlassian page
        if 'atlassian.net' in url and api_key:
            # Set the API key in the headers for Atlassian pages
...
async def process_urls(urls: List[str], max_concurrent: int = 5) -> List[str]:
    ...
    api_key = get_atlassian_api_key_from_env()
    async with async_playwright() as p:
        ...
        try:
            ...
            for i, url in enumerate(urls):
                ...
                task = fetch_page(url, context, api_key)
                tasks.append(task)
...
def main():
    # Remove the API key retrieval from command-line arguments
